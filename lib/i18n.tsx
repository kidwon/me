...
/* ============================================
   i18n — EN / ZH / JA translations + context
   Ported verbatim from the legacy i18n.js
   ============================================ */
...
const TRANSLATIONS: Record<Lang, Record<string, string>> = {
  en: {
    ...
    "exp.ntt.b2":
      "<strong>Phase 1:</strong> Refactored legacy <strong>JSP</strong> templates and intra-mart custom tags to improve front-end usability",
    "exp.ntt.b3":
      "<strong>Phase 2:</strong> Refactored business logic into clean, decoupled, testable <strong>Spring-managed beans</strong> using Dependency Injection",
    ...
    "exp.rakuten.m1":
      "Ran a high-concurrency <strong>PoC</strong> on Couchbase, generating <strong>300 million</strong> mock membership records for benchmarking",
    "exp.rakuten.m2":
      "Executed the migration from Couchbase to <strong>Cassandra</strong> after stress tests revealed degradation during node rebalancing",
    "exp.rakuten.m3":
      "Built an async <strong>Vert.x</strong> benchmarking API server and ran distributed <strong>Gatling</strong> stress tests via GitLab CI/CD",
    "exp.rakuten.m4":
      "Packaged a custom <strong>Argon2</strong> hashing library and automated API test suites with <strong>Postman/Newman</strong> on MiniKube",
    "exp.rakuten.project2": "Rakuten Fashion — Agency Order Management",
    "exp.rakuten.f1":
      "Built <strong>Spring Boot @Scheduled</strong> batch tasks with <strong>RabbitMQ</strong> to route stalled orders back for asynchronous re-processing",
    "exp.rakuten.f2":
      "Streamlined <strong>Gradle</strong> builds and API version management across microservices with <strong>JFrog Artifactory</strong>",
    "exp.rakuten.f3":
      "Standardized API specs with <strong>OpenAPI (Swagger)</strong> across all backend services",
    "exp.earlier.company": "Earlier Roles",
    "exp.earlier.role": "Software Engineer & Game Client Programmer",
    "exp.earlier.period": "2002 — 2021",
    "exp.earlier.b1":
      "<strong>Techoes, Tokyo (2020–2021):</strong> Multiplayer game backend — online chat & AI pathfinding with <strong>Go, gRPC, Protobuf</strong>; live streaming platform (Vue.js, PHP/Laravel)",
    "exp.earlier.b2":
      "<strong>NEC, Tokyo (2018–2019):</strong> VMS integrations for the National Police Agency (<strong>C# / Milestone XProtect</strong>); Mobile Suica backend APIs & batch tasks",
    "exp.earlier.b3":
      "<strong>Game Dev, Shanghai (2003–2018):</strong> Client-side development in <strong>C++/C#/Unity/Unreal</strong> across studios incl. 2K Games China — combat systems, UI frameworks, performance optimization",
    ...
    "proj.tube.desc":
      "A developer-facing SaaS that indexes massive YouTube subtitle corpora into searchable segments — query how Japanese is actually used in real videos, with millisecond latency and quota-based billing.",
    ...
    "proj.jessie.desc":
      "A React/Convex serverless web app aggregating live market data (VIX, CNN Fear & Greed, major ETFs) and classifying the market into actionable sentiment scenarios.",
    ...
    "contact.desc":
      "Currently based in Tokyo, Japan. Open to backend engineering opportunities, consulting, and interesting side projects.",
    ...
    // Chat
    "chat.open": "Ask AI about me",
    "chat.title": "ask_yuan.sh",
    "chat.greeting":
      "Hi! I'm Yuan Genggeng's AI assistant. Ask me anything about his experience, skills, or projects — in English, 中文, or 日本語.",
    "chat.placeholder": "Ask about my experience…",
    "chat.send": "Send",
    "chat.thinking": "thinking…",
    "chat.error": "Something went wrong. Please try again.",
    "chat.disclaimer": "AI-generated · may contain inaccuracies",

    // Footer
    "hero.name": 'Yuan<br /><span class="name-accent">Genggeng</span>',
    "footer.name": "Yuan Genggeng",
    "footer.copy": "Built with Next.js · Tokyo 2026",
  },
  ...
  zh: {
    ...
    "exp.ntt.b2":
      "<strong>第一阶段：</strong>重构遗留<strong>JSP</strong>模板与 intra-mart 自定义标签，优化前端可用性",
    "exp.ntt.b3":
      "<strong>第二阶段：</strong>基于依赖注入将业务逻辑重构为解耦、可测试的<strong>Spring 管理 Bean</strong>",
    "exp.rakuten.role": "合同制后端工程师",
    "exp.rakuten.period": "2021年9月 — 2025年3月",
    "exp.rakuten.project1": "会员系统与数据库迁移",
    "exp.rakuten.m1":
      "主导 Couchbase 高并发<strong>PoC</strong>验证，生成<strong>3亿条</strong>模拟会员数据进行基准测试",
    "exp.rakuten.m2":
      "在压测揭示节点再平衡期间性能劣化后，执行 Couchbase 至<strong>Cassandra</strong>的数据库迁移",
    "exp.rakuten.m3":
      "基于异步<strong>Vert.x</strong>框架开发基准测试 API 服务，并通过 GitLab CI/CD 执行分布式<strong>Gatling</strong>压测",
    "exp.rakuten.m4":
      "封装自定义<strong>Argon2</strong>哈希库，并在 MiniKube 环境用<strong>Postman/Newman</strong>自动化 API 测试套件",
    "exp.rakuten.project2": "乐天时尚 — 代理订单管理系统",
    "exp.rakuten.f1":
      "基于<strong>Spring Boot @Scheduled</strong>与<strong>RabbitMQ</strong>构建定时批处理任务，将滞留订单重新入队异步处理",
    "exp.rakuten.f2":
      "梳理各微服务的<strong>Gradle</strong>构建配置与 API 版本管理，基于<strong>JFrog Artifactory</strong>解决依赖冲突",
    "exp.rakuten.f3":
      "在所有后端服务中统一推行<strong>OpenAPI (Swagger)</strong>接口规范",
    "exp.earlier.company": "早期经历",
    "exp.earlier.role": "软件工程师 & 游戏客户端程序员",
    "exp.earlier.period": "2002 — 2021年",
    "exp.earlier.b1":
      "<strong>Techoes・东京（2020–2021）：</strong>多人游戏后端——基于<strong>Go、gRPC、Protobuf</strong>的在线聊天与 AI 寻路模块；直播平台开发（Vue.js、PHP/Laravel）",
    "exp.earlier.b2":
      "<strong>NEC・东京（2018–2019）：</strong>警察厅视频管理系统集成（<strong>C# / Milestone XProtect</strong>）；Mobile Suica 后端 API 与批处理任务",
    "exp.earlier.b3":
      "<strong>游戏开发・上海（2003–2018）：</strong>C++/C#/Unity/Unreal 客户端开发，历经 2K Games China 等多家工作室——战斗系统、UI 框架与性能优化",
    ...
    "proj.tube.desc":
      "面向开发者的商业化 SaaS：将海量 YouTube 字幕切分建立索引，通过 API 毫秒级检索日语表达在真实视频中的用法，内置配额计费与管理后台。",
    ...
    "proj.jessie.desc":
      "基于 React/Convex 的无服务端 Web 应用，聚合 VIX、CNN 恐慌贪婪指数等实时市场数据，并分类输出可操作的市场情绪信号。",
    ...
    "chat.title": "ask_yuan.sh",
    "chat.greeting":
      "你好！我是袁耿耿的 AI 助手，可以回答关于他的经历、技术栈和项目的任何问题（支持中/日/英）。",
    "chat.placeholder": "问问我的工作经历…",
    "chat.send": "发送",
    "chat.thinking": "思考中…",
    "chat.error": "出错了，请重试。",
    "chat.disclaimer": "AI 生成 · 内容仅供参考",

    // Footer
    "footer.copy": "Next.js 构建 · 东京 2026",
  },
  ...
  ja: {
    ...
    "exp.ntt.b2":
      "<strong>フェーズ1：</strong>レガシー<strong>JSP</strong>テンプレートと intra-mart カスタムタグをリファクタリングし、フロントエンドの操作性を改善",
    "exp.ntt.b3":
      "<strong>フェーズ2：</strong>DIを活用し、ビジネスロジックを疎結合でテスト可能な<strong>Spring管理Bean</strong>へリファクタリング",
    "exp.rakuten.role": "契約バックエンドエンジニア",
    "exp.rakuten.period": "2021年9月 — 2025年3月",
    "exp.rakuten.project1": "会員システム & データベース移行",
    "exp.rakuten.m1":
      "Couchbaseの高並行<strong>PoC</strong>を実施し、ベンチマーク用に<strong>3億件</strong>のモック会員データを生成",
    "exp.rakuten.m2":
      "負荷試験でノードリバランス時の性能劣化が判明したため、Couchbaseから<strong>Cassandra</strong>への移行を実施",
    "exp.rakuten.m3":
      "非同期<strong>Vert.x</strong>ベンチマークAPIサーバーを開発し、GitLab CI/CD経由で分散<strong>Gatling</strong>負荷試験を実行",
    "exp.rakuten.m4":
      "カスタム<strong>Argon2</strong>ハッシュライブラリを開発・配布し、MiniKube環境で<strong>Postman/Newman</strong>によるAPIテストを自動化",
    "exp.rakuten.project2": "楽天ファッション — 代理注文管理システム",
    "exp.rakuten.f1":
      "<strong>Spring Boot @Scheduled</strong>と<strong>RabbitMQ</strong>による定期バッチを構築し、滞留注文をキューへ再投入して非同期再処理",
    "exp.rakuten.f2":
      "各マイクロサービスの<strong>Gradle</strong>ビルド設定とAPIバージョン管理を<strong>JFrog Artifactory</strong>で整理し、依存関係の競合を解消",
    "exp.rakuten.f3":
      "全バックエンドサービスへの<strong>OpenAPI (Swagger)</strong>仕様書の統一導入を推進",
    "exp.earlier.company": "過去の経歴",
    "exp.earlier.role": "ソフトウェアエンジニア & ゲームクライアントプログラマー",
    "exp.earlier.period": "2002 — 2021年",
    "exp.earlier.b1":
      "<strong>Techoes・東京（2020–2021）：</strong>マルチプレイヤーゲームバックエンド——<strong>Go・gRPC・Protobuf</strong>によるチャット & AI経路探索モジュール；ライブ配信プラットフォーム（Vue.js、PHP/Laravel）",
    "exp.earlier.b2":
      "<strong>NEC・東京（2018–2019）：</strong>警察庁向け映像管理システム統合（<strong>C# / Milestone XProtect</strong>）；モバイルSuicaバックエンドAPI & バッチ処理",
    "exp.earlier.b3":
      "<strong>ゲーム開発・上海（2003–2018）：</strong>2K Games China等の複数スタジオにて<strong>C++/C#/Unity/Unreal</strong>クライアント開発——戦闘システム、UIフレームワーク、パフォーマンス最適化",
    ...
    "proj.tube.desc":
      "大量のYouTube字幕をセグメント化してインデックス構築し、実際の動画での日本語表現をミリ秒で検索できる開発者向け商用SaaS。クォータ課金・管理画面を完備。",
    ...
    "proj.jessie.desc":
      "VIX・CNN恐怖貪欲指数・主要ETFなどのリアルタイム市場データを集約し、市場センチメントを分類・可視化するReact/Convexサーバーレスアプリ。",
    ...
    "contact.desc":
      "現在東京在住。バックエンドエンジニアリングのポジション、技術顧問、副業プロジェクトなど幅広くご相談ください。",
    ...
    "chat.title": "ask_yuan.sh",
    "chat.greeting":
      "こんにちは！袁耿耿のAIアシスタントです。経歴・スキル・プロジェクトについて何でも聞いてください（日/中/英対応）。",
    "chat.placeholder": "経歴について質問…",
    "chat.send": "送信",
    "chat.thinking": "考え中…",
    "chat.error": "エラーが発生しました。もう一度お試しください。",
    "chat.disclaimer": "AI生成 · 内容は参考情報です",

    // Footer
    "footer.copy": "Next.js製 · 東京 2026",
  },
};
...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  // Restore stored / browser language after mount (SSR renders EN)
  useEffect(() => {
    const stored = localStorage.getItem(LANG_KEY);
    const guess = (stored || navigator.language.split("-")[0]) as Lang;
    if (LANGS.includes(guess) && guess !== "en") setLangState(guess);
  }, []);

  // Reflect language into <html lang>, <title>, meta description
  useEffect(() => {
    document.documentElement.lang =
      lang === "zh" ? "zh-CN" : lang === "ja" ? "ja" : "en";
    document.title = TRANSLATIONS[lang]["meta.title"];
    const meta = document.querySelector('meta[name="description"]');
    if (meta) meta.setAttribute("content", TRANSLATIONS[lang]["meta.desc"]);
  }, [lang]);
